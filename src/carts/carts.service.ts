...
export class CartsService{
    ...
    addToCart(cartDto: AddToCartDto) {
        ...
        if (!cartDto) {
            throw new HttpException("Invalid Parameter received", HttpStatus.BAD_REQUEST)
        }
        // check product reference is valid;
        const referencedProduct = products.find((p) => p.id === cartDto.productId);
        if (!referencedProduct) {
            throw new HttpException("Invalid product reference", HttpStatus.BAD_REQUEST)
        }
        if (referencedProduct.quantity < cartDto.quantity) {
            throw new HttpException(`Invalid product quantity. Only ${referencedProduct.quantity} left.`, HttpStatus.BAD_REQUEST)
        }
        ...
        if (cartDto.cartId) {
            cart = carts.find((c) => c.id === cartDto.cartId && c.userId === cartDto.userId);
            
            if (cart === undefined) {
                throw new HttpException("Invalid cart reference", HttpStatus.BAD_REQUEST)
            }
        ...
        cartDetail = cart.cartDetails.find((c)=> c.productId === cartDto.productId)
            if(cartDetail){
     ...
     checkout(cartId: number, userId: number) {
        let referncencedCart = carts.find((c) => c.id === cartId && c.userId === userId );
        if (!referncencedCart) {
            // throw error
     ...
     getOrders(userId: number, cartId?: number) {
        ...
        if (cartId) {
            order = orders.find((o) => o.cartId === cartId && o.cart.userId === userId);
            return order;
        }
        else {
